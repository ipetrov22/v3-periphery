...
const DAI = '0xabC2cd00700E06922BCF30fe0aD648507113CC56'
...
describe('NonfungibleTokenPositionDescriptor', () => {
  ...
  describe('#tokenRatioPriority', () => {
    ...
    it('returns 300 for DAI', async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(DAI, 1)).to.eq(300)
    })
  ...
  describe('#flipRatio', () => {
    ...
    it('returns true if token0 is a numerator and token1 is a denominator', async () => {
      expect(await nftPositionDescriptor.flipRatio(DAI, weth9.address, 1)).to.eq(true)
    })

    it('returns false if token1 is a numerator and token0 is a denominator', async () => {
      expect(await nftPositionDescriptor.flipRatio(weth9.address, DAI, 1)).to.eq(false)
    })
  })
